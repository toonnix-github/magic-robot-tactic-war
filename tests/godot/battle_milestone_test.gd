...
func _test_phase2_playable_flow() -> void:
	...
	_assert_true(editor.stat_breakdown.text.contains("COMBAT STATS"), "#58: default tab shows combat stats")
	_assert_true(editor.mech_view.cur_part_hps.has("Body"), "#FM3: mech view tracks part HPs for visual legend")
	_assert_equal(editor.mech_view.cur_part_hps["Body"], 110, "#FM3: mech view has current body HP")
	_assert_false(editor.stat_breakdown.text.contains("PARTS HP"), "#FM3: part HP moved out of panel to visual legend")
	_assert_false(editor.stat_breakdown.text.contains("Total Armor"), "#FM3: total armor removed in favor of per-part HP")
	_assert_true(editor.stat_breakdown.text.contains("PILOT SKILL"), "#FM3: pilot skill is labeled")
	...
	if editor.has_method("preview_part"):
		...
		_assert_equal(editor.mech_view.preview_part_hps["Body"], 138, "#FM3: mech view preview HPs updated for visual legend")
		...
		_assert_equal(editor.mech_view.cur_part_hps["Body"], 138, "#FM3: visual legend reflects new equipped HP")
	...
	editor._next_unit()
	_assert_equal(flow.hangar.current_unit_id, "sera", "#FM3: next_unit cycles to sera")
	editor._prev_unit()
	_assert_equal(flow.hangar.current_unit_id, "mira", "#FM3: prev_unit cycles back to mira")
	editor.slot_tab_buttons["Left Arm"].pressed.emit()
	_assert_equal(flow.hangar.highlighted_part_name, "Left Arm", "#FM3: slot tab switches active slot")
	editor.slot_tab_buttons["Head"].pressed.emit()
	_assert_equal(flow.hangar.highlighted_part_name, "Head", "#FM3: slot tab switches active slot back to Head")
	editor.category_btn_weapon.pressed.emit()
	_assert_true(editor.weapons_container.visible, "#FM3: Weapon category tab activates weapon view")
	_assert_false(editor.parts_container.visible, "#FM3: Weapon category tab hides parts view")
	editor.category_btn_part.pressed.emit()
	_assert_true(editor.parts_container.visible, "#FM3: Part category tab activates parts view")
	_assert_false(editor.weapons_container.visible, "#FM3: Part category tab hides weapon view")
