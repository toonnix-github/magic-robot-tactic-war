...
func _test_phase2_playable_flow() -> void:
	...
	editor.stats_tab_parts.pressed.emit()
	_assert_true(editor.stat_breakdown.text.contains("PARTS HP"), "#58: parts tab shows parts HP")
	editor.stats_tab_combat.pressed.emit()
	...
	if editor.has_method("preview_part"):
		...
		editor.stats_tab_parts.pressed.emit()
		_assert_true(editor.stat_breakdown.text.contains("110 -> 138"), "#FM3: modification number is displayed in PARTS HP section")
